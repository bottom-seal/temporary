...
//!! this check vm_region permission set by init or mmap, not necessar
                             ...
                             struct task_struct *child)
{
    ...
    child->mmap_next = parent->mmap_next;//next possible available addr, for mmap requesting NULL addr

    for (curr = parent->vm_regions.next;//loop through parent vm_regions
         curr != &parent->vm_regions;
         curr = curr->next) {
        struct vm_region *src =//parent
            list_entry(curr, struct vm_region, list);

        struct vm_region *dst;
        unsigned long nr_pages;

        //allocate for child vm_region structure
        ...
        //copy parent vm_region struct
        dst->start = src->start;//VA of base
        dst->length = src->length;
        dst->backing = src->backing;//backing is pointer to initrd program (else is 0)
        dst->file_size = src->file_size;//for checking how much of last page to copy
        dst->prot = src->prot;
        dst->flags = src->flags;
        dst->type = src->type;
        dst->pages = 0;//already mapped pages
        INIT_LIST_HEAD(&dst->list);

        nr_pages = src->length / PAGE_SIZE;
        //allocate for pages array
        ...
        for (unsigned long i = 0; i < nr_pages; i++) {
            if (!src->pages[i])
                continue;
            //add 1 to page refcount
            if (retain_page((void *)src->pages[i]) < 0) {
                for (unsigned long j = 0; j < i; j++) {//if any fail free all previously successed pages
                    if (dst->pages[j])
            ...
            dst->pages[i] = src->pages[i];//child vm_region shares mapping with parent (before permission error)
        }

        list_add_tail(&dst->list, &child->vm_regions);//add to child's vm_region list
    }
...
int mmap_handle_page_fault(struct pt_regs *regs)
{
    ...
    //legal path: COW or not allocated
    //COW path: vm_region records mapped
    if (region->pages[page_idx]) {
        unsigned long *pte;
        unsigned long old_page;//virt addr
        unsigned long new_page;

        pte = get_pte(current->pgd, page_va);
        //code 15 = store/AMO page fault, PTE permission set to Read only, vm_region still Writable
        //pte valid and is marked COW, the region was set as writtable
        ...
        old_page = region->pages[page_idx];//pages array stores kernel VA of the allocated page
        //page_refcount: given kernel VA, return that page's refcount
        //if page is shared path
        if (page_refcount((void *)old_page) > 1) {
            //allocate for new page
            new_page = (unsigned long)allocate(PAGE_SIZE);
            if (!new_page) {//out of memory error
                uart_puts("[Segmentation fault]: Kill Process\n");
                thread_exit_status(-1);
                return -1;
            }
            //copy content of page to allocated page for faulting process
            memcpy((void *)new_page, (void *)old_page, PAGE_SIZE);
            free((void *)old_page);//free calls dynamic_page_free, if target page has ref > 1, it just decrement and not free the page
            region->pages[page_idx] = new_page;//now faulting process's bookkeeping stores new copy of same page
        } else {
            new_page = old_page;
        }

        pte_flags = mmap_prot_to_pte(region->prot);
        //map the page table, with write permission enabled (only writable region causes COW)
        map_pages(current->pgd,
                  page_va,
                  PAGE_SIZE,
        ...
        asm volatile("sfence.vma %0, zero" :: "r"(page_va) : "memory");//provide page va: flush TLB for that page only
        uart_puts("[Permission fault]: ");
        uart_hex(addr);
        uart_puts("\n");
        return 0;
    }
    //not COW path: allocate a page and maps user VA
    //allocate 4kb page
    page = (unsigned long)allocate(PAGE_SIZE);
    if (!page) {//out of memory error
        uart_puts("[Segmentation fault]: Kill Process\n");
        thread_exit_status(-1);
        return -1;
    }
