...
void setup_vm(unsigned long dtb_pa)
{
    ...
    map_mmio_region(0x7f700000UL, 0x900000UL);
...
//given a va, get the entry in PTE level (final level), for modifying the flag
//expects 4kb leaf, not super page
unsigned long *get_pte(unsigned long *root, unsigned long va)
{
    ...
    table = root;
    for (int level = 2; level > 0; level--) {//only walks level 2 and level 1, return level 0 entry
        unsigned long pte = table[vpn[level]];//looks at PGD entry, PMD entry

        if (!(pte & PTE_V) || is_leaf_pte(pte))//check valid: entry exists OR is leaf
            return 0;
...
//given pointer to parent 4kb PTE and child 4kb PTE, copy parent pte to child, flags become read-only, marks COW
static int cow_copy_leaf(unsigned long *parent_pte,
                         unsigned long *child_pte)
{
    unsigned long pte = *parent_pte;//pointer to parent PTE
    unsigned long flags = pte & 0x3ffUL;//keeps low 10 bits

    if ((pte & PTE_U) && (flags & PTE_W)) {//become COW page only if 1. is user page 2, is writtable
        flags = (flags & ~PTE_W) | PTE_COW;//remove write flag, add COW flag
        pte = MAKE_PTE(pte_to_pa(pte), flags);//same PPN, changes flags
        *parent_pte = pte;//update parent
    }

    *child_pte = pte;//copy PTE entry to child
    return 0;
}
//given parent and child page table, COW leaf entry, allocate table entry for child then for that table COW all entry
static int cow_copy_level(unsigned long *parent,
                          unsigned long *child,
                          int level)
{
    for (int i = 0; i < ENTRIES_PER_TABLE; i++) {//scan current page table
        unsigned long pte = parent[i];

        if (!(pte & PTE_V))//skip invalid
            continue;
        //is leaf path
        if (is_leaf_pte(pte)) {
            cow_copy_leaf(&parent[i], &child[i]);//COW: if W -> parent/child becomes R only, add COW flag
            continue;
        }
        //is table path
        unsigned long *child_next = allocate(PAGE_SIZE);//if not a leaf, need to allocate a page for next level page table for child
        if (!child_next)
            return -1;

        memset(child_next, 0, PAGE_SIZE);
        child[i] = MAKE_PTE(virt_to_phys((unsigned long)child_next), PTE_V);// record a non-leaf PTE for child

        if (level > 0) {
            unsigned long *parent_next =//VA of next level page table
                (unsigned long *)phys_to_virt(pte_to_pa(pte));//VA of the entry's (PPN -> PA), addr of next level table

            if (cow_copy_level(parent_next, child_next, level - 1) < 0)//copy for next level page table
                return -1;//only if out of memory
        }
    }
...
{
    if (!parent_pgd || !child_pgd)//check roots exist
        return -1;

    for (int i = 0; i < 256; i++) {
        unsigned long pte = parent_pgd[i];//copy only user VA

        if (!(pte & PTE_V))//check entry is valid
            continue;

        if (is_leaf_pte(pte)) {//we shouldn't hava PGD leaf, for safety
            cow_copy_leaf(&parent_pgd[i], &child_pgd[i]);
            continue;
        }
        //allocate for next level table
        unsigned long *child_pmd = allocate(PAGE_SIZE);
        if (!child_pmd)
            return -1;

        memset(child_pmd, 0, PAGE_SIZE);
        child_pgd[i] = MAKE_PTE(virt_to_phys((unsigned long)child_pmd), PTE_V);//record the table entry

        unsigned long *parent_pmd =//get VA of next level table
    ...
    asm volatile("sfence.vma zero, zero" ::: "memory");//page table modified, need flushing TLB
    return 0;
}
